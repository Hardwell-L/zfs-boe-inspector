# Changelog

## npm 0.2.10 - 2026-09-24

- 移除 BOE peer 版本约束，修复 3.x、4.x 项目后缀版本触发的 npm ERESOLVE 安装冲突。
- BOE 入口在注册采集前检查主版本；非 3.x、4.x 或无法识别的版本提示一次并跳过采集。
- 同步公共类型、Adapter 依赖和 Runtime 上报版本至 `0.2.10`。

## Extension 0.2.9 - 2026-09-24

- 差旅标准页展示 3.x 当前人员逐行的页面计算金额、候选标准条件和已有字段，保留请求来源未核验提示。
- 低版本字段配置、规则诊断与过程记录按可用能力展示，避免将缺失能力误报为需要升级 Adapter。

## npm 0.2.9 - 2026-09-24

- Adapter 增加 Vue2 与 BOE 3.x 的物理入口和运行时兼容分支，保持高版本入口及安装方式不变。
- 低版本差旅采集城市标准缓存及宿主逐日计算结果；缺少请求历史时跳过无法确定的规则结论。
- 同步公共类型、Adapter 依赖和 Runtime 上报版本至 `0.2.9`。

## Extension 0.2.8 - 2026-09-18

- 过程记录按单据区域和触发条件分组展示，并为每个条件提供独立分页。
- 重复触发字段和值合并展示并标记次数，事件详情按需展开以降低大记录量渲染开销。
- 展示结构化停止原因，优化区域名称、层级间距和原生展开箭头样式。

## npm 0.2.8 - 2026-09-18

- Adapter 支持通过 `getTraceConditions` 采集运行时条件定义及事件命中结果。
- 增加同步调用深度、事件速率、事件数量和记录容量保护，并返回结构化停止原因。
- 同步公共类型、Adapter 依赖和 Runtime 上报版本至 `0.2.8`。

## Extension 0.2.7 - 2026-09-18

- 支持在模型设置中自定义 AI 请求体大小上限。
- 在问答页面展示已选字段与区域，并支持快速调整分析范围。

## npm 0.2.6 - 2026-09-18

- 支持同一区域重复字段按配置索引独立检查。
- 过程记录采集触发字段、触发值及不可用原因，支持增量读取。
- 为过程记录序列化增加节点和字符串预算，限制采集开销。
- 同步公共类型、Adapter 依赖和 Runtime 上报版本至 `0.2.6`。

## npm 0.2.3 - 2026-09-16

- 发布 Adapter 的字段与区域连续点选、选择定位和过程记录能力，供 Extension AI 分析使用。
- 补充选择范围、Bridge 能力和过程记录公共类型。
- 同步 Runtime 上报版本与 npm 包版本至 `0.2.3`。

## Extension 0.2.5 - 2026-09-04

- 规则依赖标题展示当前值摘要，优化显示文本、计算说明和动态显示规则筛选。
- 新增 AI 排障与过程分析，支持直接关联证据、请求精简、脱敏、Markdown 回答及引用定位。
- AI 分析范围支持页面字段和区域连续点选；需配合支持 `continuous-picker` 的 Adapter，旧 Adapter 保留单次选择。
- 修复 AI 配置读取和诊断列表的 TypeScript 类型检查错误。

## Extension 0.2.4 - 2026-09-02

- 更新 Runtime Adapter 未接入时的提示，与 `billTemplate.vue` 单点注册和差旅 Collector 自动注册方式保持一致。

## Extension 0.2.3 - 2026-08-28

- 扩展 Side Panel 支持访问任意 HTTP/HTTPS BOE 环境，不再局限于 `localhost` 和 `127.0.0.1`。
- 增加独立 Extension ZIP 发布流程，不更新或发布 npm 包。

## 0.2.2 - 2026-08-28

- `billTemplate` Collector 自动识别标准差旅父组件并注册 Travel Collector，业务项目无需逐个修改差旅 wrapper。
- 自动注册与显式差旅 Mixin/Composable 共享同一 Collector，并通过引用计数安全管理注销生命周期。
- 新增自动发现开关与自定义差旅宿主解析回调，兼容非标准组件层级。

## 0.2.1 - 2026-08-27

- 修复 `adapter-vue3@0.2.0` 在 npm registry 中残留 `workspace:^`、导致 npm 客户端无法安装的问题。
- npm 发布流程固定发布 `release/npm` 中的 tarball，并在发布检查中禁止 public package 使用 `workspace:` 协议。

## 0.1.2 - 2026-08-27

- 新增差旅组件的 Mixin 和 Composition API Composable，自动安装 Inspector Runtime。
- 修复 `NEW_TRAVEL_BOE` 父组件先于 `billTemplate` 子组件挂载时的 Runtime 未安装异常。

## 0.1.1 - 2026-08-27

- 修复 Adapter 发布包中 `workspace:^` 依赖导致 npm 客户端无法安装的问题。
- npm 发布包改用普通 semver 依赖，兼容 npm、pnpm 和 yarn consumer。

## 0.1.0 - 2026-08-27

- 提供 Vue3 BOE Runtime Adapter 和只读 Browser Extension。
- 支持最终单据模板、区域配置、字段配置和差旅标准检查。
- 支持页面字段选择、定位及 Side Panel 展示。
- 提供 Options API Mixin 与 Composition API `setup()` 两种接入方式。
