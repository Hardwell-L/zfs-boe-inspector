...
export function scopeAreas(snapshot: BoeInspectionSnapshot) {
  ...
  return snapshot.config.template.map((raw) => {
    ...
    return {
      ...
      fields: (Array.isArray(area.areaFields) ? area.areaFields : []).map((rawField, fieldIndex) => {
        const field = scopeRecord(rawField);
        const fieldCode = String(field.fieldCode ?? field.code ?? '');
        return { code: fieldCode, fieldIndex, label: String(field.fieldName || field.label || fieldCode), labelCode: String(field.labelCode ?? '') };
...
export function scopeIssues(snapshot: BoeInspectionSnapshot, scopes: InspectionSelection[]) {
  ...
  return [...new Set(scopes.flatMap((scope) => {
    ...
    if (scope.kind === 'field' && !area.fields.some((field) => field.code === scope.fieldCode && (scope.fieldIndex === undefined || field.fieldIndex === scope.fieldIndex))) return [`字段 ${scope.areaCode}.${scope.fieldCode} 已不在当前模板中`];
