...
import { traceChanges, traceTime, type traceEventView } from './traceView';
...
<template>
  <details class="trace-event trace-event-card" @toggle="toggle">
    <summary>
      <span class="trace-event-heading"><strong>{{ item.event.category }}</strong><span>{{ traceTime(item.event.at) }}</span><span class="trace-status" :class="{ 'trace-error': item.event.status === 'threw' }">{{ item.status }}</span></span>
      <span class="trace-event-method">{{ item.event.id }} · {{ item.event.method }}</span>
    ...
    <div v-if="opened" class="trace-event-body">
      ...
      <dl v-if="item.triggers.length" class="readable-values">
        <template v-for="(trigger, index) in item.triggers" :key="index">
          <dt>{{ trigger.name }} · {{ trigger.sourceLabel }}</dt><dd>{{ trigger.text }}</dd>
        </template>
      </dl>
